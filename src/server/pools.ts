...
function getChainPoolKey(chainId: number): string {
...
async function getAllChainPools(tvlUSD = 5000, volUSD = 500) {
  ...
  const poolsList = await Promise.all(chains.map(cid => getUniswapv3Pools(cid, 0, tvlUSD, volUSD)))
...
async function hgetPools(rc: any, chainId: number): Promise<{[key: string]: Pool}> {
  const poolsResp = await rc.hGetAll(getChainPoolKey(chainId))
...
async function syncPoolToRedis(rc: any, chainId: number, pool: Pool) {
  await rc.hSet(getChainPoolKey(chainId), pool.id, JSON.stringify(pool))
}
...
async function chainPoolsRoutine(chainId: number, opts: PoolRoutineOpts) {
  ...
  const items = await getUniswapv3Pools(chainId, opts.total, opts.tvlUSD, opts.volUSD)
  ...
  console.log('--------------------------------------------------------------------')
  console.log(`fetch ${chainName} pools used: ${tm2 - tm1} ms`)
  console.log(`${chainName} pools from graph: ${_.size(graphPools)}, pools in redis: ${_.size(redisPools)}`)
  let changed = 0, notexist = 0
  ...
  console.log(`${chainName} pools changed: `, changed, 'new pool:', notexist)
}
...
export { getChainPoolKey, getPoolInfos, runPoolRoutine };

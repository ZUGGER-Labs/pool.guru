...
import { calcPoolPrice, getPoolPrice, getUniswapv3LiquidityPools,  } from "./pools";
...
function getQuote(chainId: number, pool: LiquidityPool) {
  ...
  console.log('cannot determine pair quote:', pool.id, pool.inputTokens[0].symbol, pool.inputTokens[1].symbol)
  return 'token0'
  // throw new Error('cannot determine pair quote')
}

function isValidPrice(open: string | BigNumber | null | undefined | number,
...
function calcPoolImpernamentLoss(chainId: number, pool: LiquidityPool) {
  ...
  if (isValidPrice(price.open1D, price.close1D)) {
    pool.impermanentLoss1d = impermanentLossV3(price.open1D, price.close1D, quote, decimals)
  }
  if (isValidPrice(price.open7D, price.close7D)) {
    pool.impermanentLoss7d = impermanentLossV3(price.open7D, price.close7D, quote, decimals)
  }
  if (isValidPrice(price.open30D, price.close30D)) {
    pool.impermanentLoss30d = impermanentLossV3(price.open30D, price.close30D, quote, decimals)
  }
}
...
(async () => {
  ...
  for (let p of top20) {
    ...
    console.log(
      `${p.id} ${p.fees[0].feePercentage} ${symbol} apy1d=${p.apyBy1d?.toFixed(6)}/${p.impermanentLoss1d?.toFixed(4)} apy7d=${p.apyBy7d?.toFixed(6)}/${p.impermanentLoss7d?.toFixed(4)} apy30d=${p.apyBy30d?.toFixed(6)}/${p.impermanentLoss30d?.toFixed(4)} `+
      `fee1d=${p.avgFee1D?.toFixed(2)} fee7d=${p.avgFee7D?.toFixed(2)} tvl=${(+p.totalValueLockedUSD).toFixed(2)}`
    );
  }
